...
const traverse = steps => steps.reduce(step, [0, 0, "N"]);

module.exports.first = compose(distance, traverse, parseInput);

const mapVectors = steps =>
  steps.reduce(
    ({ vectors, loc }, instr) => {
...
const omit = (list, exclude) => list.filter(i => i !== exclude);

const intersection = ([[afx, afy], [atx, aty]], [[bfx, bfy], [btx, bty]]) => {
...
const findIntersection = ({ vectors }) =>
  vectors.length < 2
    ? null
    : takeUntilNotNull(v => takeUntilNotNull(vv => intersection(v, vv), omit(vectors, v)), vectors);
